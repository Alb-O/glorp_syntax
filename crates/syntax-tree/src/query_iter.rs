use {
	...
	tree_sitter::{Capture, InactiveQueryCursor, Node, Pattern, Query, QueryCursor, QueryMatch, RopeInput},
};
	...
	LayerState: Default,
{
	...
	pub fn current_injection(&mut self) -> (&Injection, &mut LayerState) {
		(&self.current_injection, &mut self.current_layer.state)
	}
	...
	fn exit_injection(&mut self) -> Option<(Injection, Option<LayerState>)> {
		...
		let mut layer = replace(
			...
			self.layer_manager.active_layers.remove(&self.current_injection.layer)?,
		);
...
pub trait QueryLoader<'a> {
	...
	fn are_predicates_satisfied(
		&self, _lang: Language, _match: &QueryMatch<'_, '_>, _source: RopeSlice<'_>, _locals_cursor: &ScopeCursor<'_>,
	) -> bool {
		true
	}
}
