...
impl Syntax {
	pub fn new(
		source: RopeSlice, language: Language, timeout: Duration, loader: &impl LanguageLoader,
	) -> Result<Self, Error> {
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::NoRootConfig => {
				f.write_str("`LanguageLoader::get_config` for the root layer language returned `None`")
			}
